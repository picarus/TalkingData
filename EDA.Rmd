...
```{r}
train %>% 
  group_by(ip, app, os, channel) %>% 
  filter(n()>1) %>% 
  summarize(n=n(),a=sum(is_attributed)) %>% 
  mutate(pct=a/n) -> train_distinct_all
...
```{r}
test %>% distinct(ip) %>% nrow()-> test_ip_n
test %>% distinct(app) %>% nrow() -> test_ip_n
test %>% distinct(os) %>% nrow() -> test_os_n
test %>% distinct(channel) %>% nrow() -> test_channel_n
```

```{r}
test %>% anti_join(train_ip, by="ip") -> test_ip_not
test %>% anti_join(train_app, by="app") -> test_app_not
test %>% anti_join(train_channel, by="channel") -> test_channel_not
test %>% anti_join(train_os, by="os") -> test_os_not
```
